...
// Get Repo Directory
// ------------------
// Recursive directory search for .git folder. Then, takes the directory name.
var getRepoDirectory = function (dir, callback) {
	var file = path.resolve(dir, '.git');
	...
	fs.exists(path.join(dir, '.git'), function (exists) {
		if (exists) {
			...
			if (dir === '/') {
				return callback(new Error('Could not find .git directory.'), null);
			}
			return getRepoDirectory(path.resolve(dir, '..'), callback);
		}
	});
};
...
exports.run = function (options) {
		...
		async.series([
			...
			// Get repo name (based on directory name).
			function (done) {
				getRepoDirectory(__dirname, function (err, result) {
					if (err) {
						return done(err);
					}
					options.repo = path.basename(result);
